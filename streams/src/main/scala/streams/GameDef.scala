...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    /** Returns the list of blocks that can be obtained by moving the current block, together with the corresponding move. */
    ...
    def legalNeighbors: List[(Block, Move)] = {
      neighbors.filter { case (newBlock, _) => newBlock.isLegal }
    }
